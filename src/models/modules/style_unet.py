...
def init_styleunet(model: nn.Module):
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            nn.init.kaiming_normal_(m.weight, a=0.2, mode='fan_in', nonlinearity='leaky_relu')  # :contentReference[oaicite:0]{index=0}
            if m.bias is not None:
                nn.init.zeros_(m.bias)
        elif isinstance(m, nn.BatchNorm2d):
            nn.init.ones_(m.weight)
            nn.init.zeros_(m.bias)  # :contentReference[oaicite:1]{index=1}
        elif isinstance(m, nn.Linear):
            nn.init.xavier_uniform_(m.weight)  # :contentReference[oaicite:2]{index=2}
            nn.init.zeros_(m.bias)
        elif isinstance(m, ResBlock):
            for conv in [m.conv1, m.conv2, m.skip]:
                nn.init.orthogonal_(conv.weight)
                if conv.bias is not None:
                    nn.init.zeros_(conv.bias)
    # StyleGAN2 子模块
    default_init_weights(model.stylegan_decoder.style_mlp, scale=1, bias_fill=0, mode='fan_in', nonlinearity='leaky_relu')
    default_init_weights(model.stylegan_decoder.style_convs, scale=1, bias_fill=1, mode='fan_in', nonlinearity='linear')
    default_init_weights(model.stylegan_decoder.to_rgbs, scale=1, bias_fill=1, mode='fan_in', nonlinearity='linear')
    # SFT 子网
    for net, is_scale in [(model.condition_scale, True), (model.condition_shift, False)]:
        for layer in net:
            if isinstance(layer, nn.Conv2d):
                nn.init.kaiming_normal_(layer.weight, a=0.2, mode='fan_in', nonlinearity='leaky_relu')
                nn.init.constant_(layer.bias, 1 if is_scale else 0)
...
class StyleUNet(nn.Module):
    def __init__(
        ...
        channels = {
            ...
            '64': 512, '128': 256, '256': 128, '512': 128, '1024': 128
        }
        ...
        if self.in_size <= self.out_size:
                self.conv_body_first = nn.Sequential(
...
class StyleGAN2GeneratorCSFT(nn.Module):
    ...
    def __init__(
        ...
        channels = {
            ...
            '64': 1024, '128': 512, '256': 256, '512': 32, '1024': 16
        }
