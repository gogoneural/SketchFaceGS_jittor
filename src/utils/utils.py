...
def vis_depth(depth_map, color_map='magma', non_linear=False):
    ...
    if non_linear:
        ...
        depth_map = depth_map - depth_map.min()
        depth_map = depth_map / depth_map.max()
    original_dim = len(depth_map.shape)
    assert original_dim in [2, 3], len(depth_map.shape)
    if original_dim == 2:
        depth_map = depth_map[None]
    depth_color = matplotlib.colormaps[color_map]
    depth_map = jt.array(
        depth_color(depth_map.numpy())[..., :3]
    ).permute(0, 3, 1, 2)
    if original_dim == 2:
...
class SHEncoding(nn.Module):
    ...
    def execute(self, directions):
        ...
        components = jt.zeros((*directions.shape[:-1], num_components))
...
def positional_encoding(x, min_deg=0, max_deg=4, append_identity=True):
    ...
    scales = 2 ** jt.arange(min_deg, max_deg).float32()
    shape = x.shape[:-1] + (-1,)
    scaled_x = (x[..., None, :] * scales[:, None]).reshape(*shape)
    # Note that we're not using safe_sin, unlike IPE.
    four_feat = jt.sin(
        jt.concat([scaled_x, scaled_x + 0.5 * math.pi], dim=-1))
    if append_identity:
...
def correct_color(img, ref, num_iters=5, eps=0.5 / 255):
    ...
    for _ in range(num_iters):
        ...
        a_mat.append(jt.ones_like(img_mat[:, :1]))  # Bias term.
        a_mat = jt.concat(a_mat, dim=-1)
        warp = []
        for c in range(num_channels):
            b = ref_mat[:, c]
            mask = mask0[:, c] & is_unclipped(img_mat[:, c]) & is_unclipped(b)
            ma_mat = jt.ternary(mask[:, None], a_mat, jt.zeros_like(a_mat))
            mb = jt.ternary(mask, b, jt.zeros_like(b))
            # Use numpy lstsq as jittor doesn't have linalg.lstsq
            w_np = np.linalg.lstsq(ma_mat.numpy(), mb.numpy(), rcond=-1)[0]
            w = jt.array(w_np)
            assert jt.all(jt.isfinite(w)).item()
            warp.append(w)
        warp = jt.stack(warp, dim=-1)
        img_mat = jt.clamp(matmul(a_mat, warp), 0, 1)
...
def render_points(points, E=15, A=45, color_map='magma', scale=20, alpha=0.1, no_aixs=False):
    assert len(points.shape) == 3 and points.shape[-1] == 3, points.shape
    import io
    import matplotlib
    ...
    for point in points:
        ...
        buf.seek(0)
        plt.close()
        img_arr = np.array(Image.open(buf))[:, :, :3]  # HWC, uint8
        img_tensor = jt.array(img_arr).permute(2, 0, 1)  # CHW
