...
class FacePerceptualLoss(nn.Module):
    ...
    def forward(self, img0, img1, normalize=False,gram=False):
        self.eval()
        self.vggface.eval()
        ...
        if gram:
            ...
            for lid in range(self.L):
                feats2, feats3 = self.normalize_tensor(features_vggface_input[self.face_layers[lid]]), self.normalize_tensor(features_vggface_target[self.face_layers[lid]])
                G0 = self._gram_matrix(feats2)
                G1 = self._gram_matrix(feats3)
                loss_gram_vgg += torch.nn.functional.mse_loss(G0, G1) #if self.percep.loss_type=='l2' else torch.nn.functional.l1_loss(G0, G1)

            loss_gram_alex = 0.
            # AlexNet 特征在 outs0 和 outs1 中交叉组成 L 层
            for lid in range(self.L):
                feats0, feats1 = self.normalize_tensor(outs0[lid]), self.normalize_tensor(outs1[lid])
                G0 = self._gram_matrix(feats0)
                G1 = self._gram_matrix(feats1)
                loss_gram_alex += torch.nn.functional.mse_loss(G0, G1) #if self.percep.loss_type=='l2' else torch.nn.functional.l1_loss(G0, G1)
            return final_loss, loss_gram_vgg, loss_gram_alex
        return final_loss
    ...
    def _gram_matrix(self,feat: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
            """
            计算单个特征图的 Gram 矩阵
            feat: [B, C, H, W]
            返回: [B, C, C]
...
class AlexNet(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        super().__init__()
        weights_path = 'assets/alexnet-owt-4df8aa71.pth'
        # alexnet_layers = torchvision.models.alexnet(weights="DEFAULT").features
        alexnet = torchvision.models.alexnet()
        alexnet.load_state_dict(torch.load(weights_path))  # 从本地加载权重
        alexnet_layers = alexnet.features
...
def _vgg_face(layers):
    ...
    state_dict = torch.load( "assets/vgg_face_dag.pth"
    )
