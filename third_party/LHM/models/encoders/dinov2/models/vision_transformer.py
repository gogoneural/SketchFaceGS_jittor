...
def _trunc_normal_(tensor, std=0.02):
    ...
    with jt.no_grad():
        size = tensor.shape
        tensor_data = jt.init.gauss(size, dtype='float32') * std
        tensor_data = jt.clamp(tensor_data, -2 * std, 2 * std)
...
class DinoVisionTransformer(nn.Module):
    ...
    def init_weights(self):
        ...
        init.gauss_(self.cls_token, mean=0.0, std=1e-6)
        if self.register_tokens is not None:
            init.gauss_(self.register_tokens, mean=0.0, std=1e-6)
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        return jt.concat((class_pos_embed.unsqueeze(0), patch_pos_embed), dim=1).cast(previous_dtype)
