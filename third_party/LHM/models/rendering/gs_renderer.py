...
# Jittor doesn't have nn.ModuleDict — provide a simple polyfill
if not hasattr(nn, 'ModuleDict'):
    class _ModuleDict(nn.Module):
        ...
        def values(self):
            return (getattr(self, k) for k in self._keys_list)
        def items(self):
            return ((k, getattr(self, k)) for k in self._keys_list)
    nn.ModuleDict = _ModuleDict

# torch is needed by GS3DRenderer (not by GSLayer).
# Provide a minimal stub so the class definition doesn't crash in Jittor-only envs.
try:
    import torch
except ImportError:
    ...
    torch.autocast = lambda **kw: _noop_ctx()
    torch.zeros = lambda *a, **kw: None
    torch.zeros_like = lambda *a, **kw: None
    torch.ones = lambda *a, **kw: None
    torch.ones_like = lambda *a, **kw: None
    torch.eye = lambda *a, **kw: None
    torch.stack = lambda *a, **kw: None
    torch.cat = lambda *a, **kw: None
    torch.from_numpy = lambda *a, **kw: None
    torch.nn = _types.ModuleType("torch.nn")
    torch.nn.functional = _types.ModuleType("torch.nn.functional")
    torch.nn.functional.normalize = lambda *a, **kw: None
...
try:
    from utils.sh_utils import C0, eval_sh  # JGaussian
except ImportError:
    from LHM.models.rendering.utils.sh_utils import C0, eval_sh
...
def aabb(xyz):
    return jt.min(xyz, dim=0), jt.max(xyz, dim=0)
...
def intrinsic_to_fov(intrinsic, w, h):
    ...
    fov_x = 2 * jt.arctan2(w, 2 * fx)
    fov_y = 2 * jt.arctan2(h, 2 * fy)
    return fov_x, fov_y
...
class Camera:
    ...
    def from_c2w(c2w, intrinsic, height, width):
        ...
        FoVx, FoVy = intrinsic_to_fov(
            intrinsic,
            w=jt.array(width).float32(),
            h=jt.array(height).float32(),
        )
        return Camera(
            w2c=w2c,
            intrinsic=intrinsic,
...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.rotation_activation = lambda x, **kw: x / (jt.norm(x, dim=-1, keepdim=True) + 1e-12)

        # rgb activation function
        self.rgb_activation = jt.sigmoid
    ...
    def save_ply(self, path):
        ...
        opacities = (
            inverse_sigmoid(jt.clamp(self.opacity, 1e-3, 1 - 1e-3))
            .numpy()
        )
    ...
    def load_ply(self, path):
        ...
        xyz = jt.array(xyz)
        opacities = jt.array(opacities)
        rotation = jt.array(rots)
        scales = jt.array(scales)
        features_dc = jt.array(features_dc)
        features_rest = jt.array(features_extra)

        shs = jt.concat([features_dc, features_rest], dim=2)
        ...
        for key, out_ch in self.attr_dict.items():
            ...
            if not (key == "shs" and use_rgb):
                if key == "opacity" and self.fix_opacity:
                    ...
                    jt.init.constant_(layer.weight, 0)
                    jt.init.constant_(layer.bias, 0)
            # if key == "scaling":
                # jt.init.constant_(layer.bias, init_scaling)
            elif key == "rotation":
                if not self.fix_rotation:
                    jt.init.constant_(layer.bias, 0)
                ...
                if not self.fix_opacity:
                    jt.init.constant_(layer.bias, inverse_sigmoid(init_density))
            self.out_layers[key] = layer

        if self.use_fine_feat:
            fine_shs_layer = nn.Linear(in_channels, shs_out_ch)
            jt.init.constant_(fine_shs_layer.weight, 0)
            jt.init.constant_(fine_shs_layer.bias, 0)
            self.out_layers["fine_shs"] = fine_shs_layer

    def hyper_step(self, step):
        self.clip_scaling = self.clip_scaling_pruner.get_value(step)

    def execute(self, x, pts, x_fine=None):
        assert len(x.shape) == 2
        # Workaround: break Jittor op fusion to avoid JIT codegen bug
        # (undefined op0_outputstride in scalar broadcast kernel)
        jt.sync_all()
        ret = {}
        for k in self.attr_dict:
            ...
            v = layer(x)
            if k == "rotation":
                v = v / (jt.norm(v, dim=-1, keepdim=True) + 1e-10)
            elif k == "scaling":
                v = -(v - 5)
                v = -3 - nn.softplus(v - 3.)
                if self.clip_scaling is not None:
                    v = jt.clamp(v, min_v=0, max_v=self.clip_scaling)
            elif k == "opacity":
                if self.fix_opacity:
                    v = jt.ones_like(x)[..., 0:1]
                else:
                    v = v
            elif k == "shs":
               v = self._apply_color_activation(v)
               v = v.reshape(v.shape[0], -1, 3)
            elif k == "xyz":
                v = self._apply_position_activation(v)
                k = "offset_xyz"
            ret[k] = v

        return GaussianAppOutput(**ret)

    def _apply_color_activation(self, value):

        color_value = value[..., :3]
        color_value = mip_tanh(color_value, overshoot=0.001)
        color_value = color_value * (0.5 / C0)

        sh_value = value[..., 3:]
        sh_value = mip_tanh(sh_value, overshoot=0.001)
        sh_value = sh_value * (0.5 / C0)

        value = jt.concat([color_value, sh_value], dim=-1)

        return value

    def _apply_position_activation(self, value):
        value = 0.25 * jt.tanh(value)
        return value

    def _apply_opacity_activation(self, value):
...
class PointEmbed(nn.Module):
    def __init__(self, hidden_dim=48, dim=128):
        ...
        e = jt.pow(jt.float32(2), jt.arange(self.embedding_dim // 6).float32()) * np.pi
        e = jt.stack(
            [
                jt.concat(
                    [
    ...
    def embed(input, basis):
        projections = jt.einsum("bnd,de->bne", input, basis)
        embeddings = jt.concat([jt.sin(projections), jt.cos(projections)], dim=2)
    ...
    def execute(self, input):
        # input: B x N x 3
        embed = self.mlp(
            jt.concat([self.embed(input, self.basis), input], dim=2)
        )  # B x N x C
