...
def main(run_name: str,
    ...
    move_z = 2.7
    random_thetas = np.random.uniform(0, 2 * np.pi, 96)

    # 让 distance 在一个合理范围内波动
    random_distances = np.sqrt(np.random.uniform(0, 0.6 * move_z / 2.7, 96)+1e-6)
    poses = [circle_around_axis(
        2, up=Vec3(0, 1, 0), move=Vec3(0, 0, move_z),
        distance=d, theta_to=theta
    )[1] for theta, d in zip(random_thetas, random_distances)]
