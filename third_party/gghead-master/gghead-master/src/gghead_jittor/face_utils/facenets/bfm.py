...
def transferBFM09(bfm_folder='BFM'):
    ...
    # save our face model
    savemat(osp.join(bfm_folder, 'BFM_model_front_seg.mat'), {'meanshape': meanshape, 'meantex': meantex, 'idBase': idBase, 'exBase': exBase, 'texBase': texBase,
...
class ParametricFaceModel(nn.Module):
    def __init__(self, 
        ...
        # seg_mask[:,0] for eye, [:2] for mouth
        # self.seg_mask = model['segbin'].astype(bool)
    ...
    def render_with_pose(self, coeffs, pose):
        """
        Return:
            face_vertex     -- torch.tensor, size (B, N, 3), in camera coordinate
            face_color      -- torch.tensor, size (B, N, 3), in RGB order
        ...
        face_shape = self.compute_shape(coef_dict['id'], coef_dict['exp'])
        rotation = self.compute_rotation(pose)
        face_shape_transformed = self.transform(face_shape, rotation, coef_dict['trans'])
