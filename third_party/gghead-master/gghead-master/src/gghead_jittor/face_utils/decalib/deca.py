...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
                render_orig=False, original_image=None, tform=None, eye_pose=None):
        if 'images' in codedict:
        ...
        ## decode
        codedict['pose'][:, :3] = 0
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'], eye_pose_params=eye_pose)
        ...
        if vis_lmk:
            ...
            ### debug:
            # landmarks3d_vis = torch.cat([landmarks3d_vis, torch.tensor([1]).view(1, 1, 1).to(landmarks3d_vis)], 1)
    ...
    def save_obj_wo_eyeball(self, filename, opdict):
        '''
        ...
        faces = faces[np.all(faces < _V, axis=1)]
        faces = faces[:, ::-1]
        util.write_obj(filename, vertices[:_V], faces,)
    ...
    def save_obj(self, filename, opdict):
        ...
        vertices = opdict['verts'][i].cpu().numpy()
        faces = self.render.faces[0].cpu().numpy()
        texture = util.tensor2image(opdict['uv_texture_gt'][i]) if 'uv_texture_gt' in opdict else None
        uvcoords = self.render.raw_uvcoords[0].cpu().numpy()
        uvfaces = self.render.uvfaces[0].cpu().numpy()
        # save coarse mesh, with texture and normal map
        normal_map = util.tensor2image(opdict['uv_detail_normals'][i]*0.5 + 0.5) if 'uv_detail_normals' in opdict else None
...
class DECA_E(nn.Module):
    def __init__(self, cfg=cfg, device='cuda'):
        super(DECA_E, self).__init__()
        self.cfg = cfg
        self.device = device
        
        self.n_param = cfg.model.n_shape+cfg.model.n_tex+cfg.model.n_exp+cfg.model.n_pose+cfg.model.n_cam+cfg.model.n_light
        self.net = ResnetEncoder(outsize=self.n_param).to(self.device)
        model_path = self.cfg.pretrained_modelpath
        if os.path.exists(model_path):
            checkpoint = torch.load(model_path)
            self.checkpoint = checkpoint
            util.copy_state_dict(self.net.state_dict(), checkpoint['E_flame'])
        else:
            raise ValueError(f'please check model path: {model_path}')
        self.net.eval()
