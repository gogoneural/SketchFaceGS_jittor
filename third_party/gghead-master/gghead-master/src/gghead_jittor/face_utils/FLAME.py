...
class FLAME(nn.Module):
    ...
    def __init__(self, config=cfg):
        ...
        self.config = config
        print(f"creating the FLAME Decoder, {config.flame_model_path}")
        ...
        self.register_buffer('eye_pose', default_eyball_pose)
        default_neck_pose = torch.zeros([1, 3], dtype=self.dtype, requires_grad=False)
        self.register_buffer('neck_pose', default_neck_pose)

        # Static and Dynamic Landmark embeddings for FLAME
        lmk_embeddings = np.load(config.flame_lmk_embedding_path, allow_pickle=True, encoding='latin1')
        lmk_embeddings = lmk_embeddings[()]
        self.register_buffer('lmk_faces_idx', torch.from_numpy(lmk_embeddings['static_lmk_faces_idx']).long().contiguous())
        self.register_buffer('lmk_bary_coords', torch.from_numpy(lmk_embeddings['static_lmk_bary_coords']).to(self.dtype).contiguous())
        ...
        self.register_buffer('full_lmk_faces_idx', torch.from_numpy(lmk_embeddings['full_lmk_faces_idx']).long().contiguous())
        self.register_buffer('full_lmk_bary_coords', torch.from_numpy(lmk_embeddings['full_lmk_bary_coords']).to(self.dtype).contiguous())
        ...
        self.register_buffer('neck_kin_chain', torch.stack(neck_kin_chain).contiguous())
