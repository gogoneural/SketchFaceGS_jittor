...
import trimesh
from src.utils.dreifus_compat import PoseType, CameraCoordinateConvention, Pose, Intrinsics

from elias.config import Config, implicit

from gghead_jittor.constants import DEFAULT_INTRINSICS
from gghead_jittor.config.gaussian_attribute import GaussianAttribute, GaussianAttributeConfig
from gghead_jittor.env import REPO_ROOT_DIR
from gghead_jittor.models.stylegan2 import GGHGenerator as GGHStyleGAN2Backbone, GGHSynthesisNetwork, GGHSynthesisBlock
from gghead_jittor.util.activation import mip_tanh, mip_sigmoid, mip_tanh2
from gghead_jittor.util.logging import LoggerBundle
from gghead_jittor.util.mesh import gaussians_to_mesh
from gghead_jittor.util.rotation import axis_angle_to_quaternion
from gghead_jittor.util.uv import gen_tritex

# --- Add JGaussian-main to sys.path for GaussianModel, render, eval_sh ---
# Priority: 1) JGAUSSIAN_PATH env var  2) relative path from source tree  3) already on sys.path
_jg_path = os.environ.get('JGAUSSIAN_PATH', None)
if _jg_path is None:
    # Try relative path (works in dev / editable install from source tree)
    _jg_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', 'JGaussian-main'))
if _jg_path and os.path.isdir(_jg_path) and _jg_path not in sys.path:
    sys.path.insert(0, _jg_path)

from gaussian_renderer import render
from scene.gaussian_model import GaussianModel
from utils.sh_utils import C0, eval_sh

# --- Inline utilities (from gaussian_splatting / eg3d / dreifus) ---
...
def decode_camera_params(camera_params, disable_rotation_check=False):
...
def to_homogeneous(points):
    ones = jt.ones_like(points[..., :1])
    return jt.concat([points, ones], dim=-1)

def _getWorld2View2(R, t, translate=np.array([.0, .0, .0]), scale=1.0):
    Rt = np.zeros((4, 4))
    Rt[:3, :3] = R.transpose()
    Rt[:3, 3] = t
    Rt[3, 3] = 1.0
...
def _getProjectionMatrix(znear, zfar, fovX, fovY, width, height, cx=None, cy=None):
    ...
    offset_x = (cx - (width / 2)) / focal_x * znear
    offset_y = (cy - (height / 2)) / focal_y * znear
    top = top + offset_y
    left = left + offset_x
...
class RenderCam:
    def __init__(self, width, height, R, T, FoVx, FoVy, cx=None, cy=None,
                 znear=0.01, zfar=100, trans=np.array([0.0, 0.0, 0.0]), scale=1.0):
        self.FoVx = FoVx
        self.FoVy = FoVy
        self.image_width = width
        self.image_height = height
        self.world_view_transform = jt.array(_getWorld2View2(R, T, trans, scale)).transpose(0, 1)
        projection_matrix = _getProjectionMatrix(
            znear=znear, zfar=zfar, fovX=FoVx, fovY=FoVy,
            width=width, height=height, cx=cx, cy=cy).transpose(0, 1)
        self.full_proj_transform = jt.bmm(self.world_view_transform.unsqueeze(0),
            projection_matrix.unsqueeze(0)).squeeze(0)
        self.camera_center = jt.linalg.inv(self.world_view_transform)[3, :3]

def pose_to_rendercam(pose, intrinsics, img_w, img_h, znear=0.01, zfar=100, **kwargs):
    ...
    return RenderCam(img_w, img_h, R, T, fov_x, fov_y, cx=cx, cy=cy, znear=znear, zfar=zfar)
...
class GGHeadConfig(Config):
    ...
    background_cylinder_angle: float = np.pi / 2  # Angle of the cylinder patch if curve_background_plane=True. Larger angle = larger background plane
...
class GaussianAttributeOutput:
    gaussian_attributes: Dict[GaussianAttribute, jt.Var]

    # Needed for regularization
    raw_gaussian_attributes: Optional[Dict[GaussianAttribute, jt.Var]] = None

    # Diagnostics
    uv_map: Optional[jt.Var] = None  # [B, S, UV, H_f, W_f]
    background_uv_map: Optional[jt.Var] = None
    auxiliary_gaussian_attributes: Optional[Dict[GaussianAttribute, jt.Var]] = None
    raw_auxiliary_gaussian_attributes: Optional[Dict[GaussianAttribute, jt.Var]] = None


@dataclass
class GGHeadOutput(dict):
    images: jt.Var  # [B, 3, H, W] in [-1, 1]
    images_raw: jt.Var  # [B, 3, H_raw, W_raw] in [-1, 1]
    images_depth: jt.Var  # [B, H, W]
    gaussian_attribute_output: GaussianAttributeOutput
    masks: Optional[jt.Var] = None  # [B, 1, H, W] in [-1, 1]
    backgrounds: Optional[jt.Var] = None  # [B, 3, H, W] in [-1, 1]
    images_features: Optional[jt.Var] = None
    planes: jt.Var = None
    sketchs: jt.Var = None
    camera: Optional[List] = None  # 改为列表类型

    def __getitem__(self, key):
        if key == 'image':
            images = self.images
            if self.masks is not None:
                images = jt.concat([images, self.masks], dim=1)
            if self.backgrounds is not None:
                images = jt.concat([images, self.backgrounds], dim=1)
            return images
        elif key == 'image_raw':
        ...
        elif key == 'gaussian_attribute_output':
            return self.gaussian_attribute_output
        elif key == 'camera':  # 处理 camera 列表
            if self.camera is None:
                raise ValueError("Camera parameters have not been set.")
            return self.camera
        else:
    ...
    def __setitem__(self, key, value):
        if key == 'image':
            ...
            self.planes = value
        elif key == 'camera':  # 允许设置 camera 作为列表
            if not isinstance(value, list):
                raise TypeError("Camera must be a list of jt.Var.")
            self.camera = value
        elif key == 'gaussian_attribute_output':
            self.gaussian_attribute_output = value
        else:
            raise ValueError(f"Unknown key: {key}")

    def keys(self):
        return iter(['image', 'image_raw', 'image_depth', 'camera','gaussian_attribute_output','planes','sketchs'])
...
class GGHeadModel(nn.Module):
    ...
    def __init__(self, config: GGHeadConfig, logger_bundle: Optional[LoggerBundle] = None,
        ...
        if config.use_zero_conv_position:
            ...
            zero_conv_position.weight.assign(jt.zeros_like(zero_conv_position.weight))
            zero_conv_position.bias.assign(jt.zeros_like(zero_conv_position.bias))
        ...
        # Store as Jittor Vars (originally register_buffer in PyTorch)
        self._idxim = jt.array(idxim).stop_grad()
        self._barim = jt.array(barim.astype(np.float32)).stop_grad()
        ...
        xs = jt.linspace(-1, 1, self._config.n_uniform_flame_vertices)
        ys = jt.linspace(-1, 1, self._config.n_uniform_flame_vertices)

        xs, ys = jt.meshgrid(xs, ys)
        sampled_uv_coords = jt.stack([ys, xs], dim=-1)

        jt_position_map = jt.array(np.asarray(flame_position_map, dtype=np.float32)).permute(2, 0, 1)  # [3, H_map, W_map]
        jt_face_index_map = jt.array(np.asarray(idxim, dtype=np.int64)).permute(2, 0, 1)
        valid_uv_map = (jt_face_index_map > 0).any(dim=0).float()[None]  # [1, H_map, W_map]

        valid_samples = grid_sample(valid_uv_map.unsqueeze(0), sampled_uv_coords.unsqueeze(0),
        ...
        self._uv_grid = uv_grid.contiguous()
        self._uv_position_map = jt_position_map.contiguous()

        sampled_positions = grid_sample(jt_position_map.unsqueeze(0), uv_grid,
                                        align_corners=config.use_align_corners,
                                        mode=config.interpolation_mode)[0, :, :, 0].transpose(0, 1)  # [G, 3]
        ...
        if self._config.use_flame_to_bfm_registration:
            ...
            flame_to_bfm_neutral_37_face_only = jt.array([
            ...
            flame_vertices = (to_homogeneous(flame_vertices) @ flame_to_bfm_neutral_37_face_only.transpose(0, 1))[..., :3]

            if hasattr(self, "_uv_position_map"):
                self._uv_position_map = (to_homogeneous(
                    self._uv_position_map.permute(1, 2, 0)) @ flame_to_bfm_neutral_37_face_only.transpose(0, 1))[..., :3].permute(2,
                                                                                                                    0,
                                                                                                                    1).contiguous()

        flame_vertices = flame_vertices.unsqueeze(0)  # [1, G, 3]
        self._flame_vertices = flame_vertices.contiguous()
        ...
        gaussian_bg = jt.array([1.0 for _ in range(config.gaussian_attribute_config.n_color_channels)])
        gaussian_bg_train = jt.array(list(self._config.background_color)).float32() / 255
        self._gaussian_bg = gaussian_bg
        self._gaussian_bg_train = gaussian_bg_train
    ...
    def gghead_updata(self):

        # unique_grid = torch.unique(self._uv_grid.squeeze(0).squeeze(1), dim=0)
        # xs = torch.linspace(-1, 1, steps=self._config.n_uniform_flame_vertices)
        # print('xs',xs)
        # print('xs.shape',xs.shape)
        # print('unique_grid',unique_grid.shape)
        # print('self._uv_grid',self._uv_grid.shape)
        # print('self._uv_grid',self._uv_grid)
        # torch.set_printoptions(precision=8)
        # print('True?',(self._uv_grid.squeeze(0).squeeze(1)[-1,0]-self._uv_grid.squeeze(0).squeeze(1)[-2,0]),(2/511),(self._uv_grid.squeeze(0).squeeze(1)[-1,0]-self._uv_grid.squeeze(0).squeeze(1)[-2,0])==(2/511))
        # torch.set_printoptions(precision=4)
        # print(xs[1]-xs[0])
        # print(xs[2]-xs[1])
        self._uv_idx = ((self._uv_grid-(-1))/(2/511)).squeeze(0).squeeze(1).round().long()
        unique_uv = jt.unique(self._uv_idx, dim=0)
        # print('unique_uv',unique_uv.shape)
        ...
        # idxim, tidxim, barim = gen_tritex(uv_coords, faces, uv_faces, plane_resolution)
        #
        # self.register_buffer("_idxim", jt.Var(idxim).contiguous())
        # self.register_buffer("_barim", jt.Var(barim).contiguous())
        # print(f"坐标数: {flame_template_mesh.vertices.shape}")
        # print(f"唯一坐标数: {len(np.unique(flame_template_mesh.vertices, axis=0))}")
        # import pdb
        # pdb.set_trace()
        ...
        # # **去除重复顶点**
        # unique_vertices,indices, inverse_indices = np.unique(vertices, axis=0, return_index=True,return_inverse=True)
        # new_uvs = uvs_per_flame_vertex[indices]
        # new_faces = inverse_indices[faces]
        # # 重新映射面索引

        combined = np.hstack([vertices, uvs_per_flame_vertex])
        unique_combined, inverse_indices = np.unique(combined, axis=0, return_inverse=True)

        # 分离唯一顶点和 UV
        unique_vertices = unique_combined[:, :3]
        unique_uvs = unique_combined[:, 3:5]
        ...
        # pdb.set_trace()
        # 重新创建 mesh
        # cleaned_mesh = trimesh.Trimesh(vertices=unique_vertices, faces=new_faces, process=False)

        # 重新赋值 UV 信息
        # cleaned_mesh.visual.uv = new_uvs
        # cleaned_mesh.visual = trimesh.visual.TextureVisuals(
        #     uv=unique_uvs,  # UV坐标数组
        #     material=None # 可留空材质
        # )

        # 导出为OBJ文件（自动处理顶点、UV及面片索引）
        # mesh.export("output.obj", include_uv=True)
        # cleaned_mesh.export(f"/mnt/data/kang/clean_mesh.obj", include_texture=True)
        # 验证 mesh
        print(f"去重前顶点数: {len(vertices)},  去重后顶点数: {len(unique_vertices)}")
        print(f"去重前面数: {len(faces)},  处理后面数: {len(new_faces)}")
        self._faces = jt.array(new_faces.astype(np.int64)).stop_grad()
    def sample_z(self, person_ids: jt.Var) -> jt.Var:
        if self._config.use_autodecoder:
            z = self._identity_codes(person_ids)
        else:
            z = jt.randn((len(person_ids), self._config.z_dim))
    ...
    def sample_uv_map(self, planes: jt.Var, uv_grid: jt.Var, n_shells: int = 1):
        ...
        if self._config.use_zero_conv_position and GaussianAttribute.POSITION in self._uv_attribute_names:
            ...
            uv_map = jt.concat([uv_map[:, :position_start_channel],
                                zeroed_positions,
    ...
    def predict_planes(self, ws: jt.Var, cache_backbone=False, use_cached_backbone=False,
                       alpha_plane_resolution: Optional[float] = None,return_xblock=False, **synthesis_kwargs):
        # Predict 2D planes
        if use_cached_backbone and self._last_planes is not None:
            planes = self._last_planes
        elif return_xblock:
            x_block,planes,planes_sketch = self.backbone.synthesis(ws, return_xblock=return_xblock, **synthesis_kwargs)
        else:
            planes,planes_sketch = self.backbone.synthesis(ws, return_xblock=return_xblock, **synthesis_kwargs)
        if cache_backbone:
            self._last_planes = planes
        if return_xblock:
            return x_block,planes,planes_sketch
        return planes,planes_sketch
    ...
    def predict_gaussian_attributes(self,
        ...
        if self.training and self._logger_bundle is not None:
            self._logger_bundle.log_metrics({
                ...
                "Analyze/angle_gaussian_rotations": 2 * jt.acos(gaussian_rotations[..., 0]),
    ...
    def _log_gradients(self, name: str, tensor: jt.Var):
    ...
    def get_uv_rendering(self, c: jt.Var, output: GGHeadOutput,
                         include_transparent_gaussians: bool = False) -> jt.Var:
        B = len(c)
        ...
        uv_colors = jt.concat([uv_colors, -jt.ones((1, uv_colors.shape[1], 1))],
                              dim=-1)  # [1, G, 3]
        debug_gaussian_attributes[GaussianAttribute.COLOR] = uv_colors.repeat(B, 1, 1)
        if include_transparent_gaussians:
            debug_gaussian_attributes[GaussianAttribute.OPACITY] = jt.ones_like(
        ...
        for i in range(B):
            ...
            pose, intrinsics = decode_camera_params(c_np[i])
            intrinsics = intrinsics.rescale(resolution)
            gs_cam = pose_to_rendercam(pose, intrinsics, resolution, resolution)

            uv_render = render(gs_cam, gaussian_model, PipelineParams2(), jt.array([1., 1., 1.]),
                               override_color=uv_colors[0])
            all_uv_renders.append(uv_render['render'])

        all_uv_renders = jt.stack(all_uv_renders)
    ...
    def get_gaussian_mesh(self,
                          ...
                          overwrite_colors: Optional[jt.Var] = None,
        ...
        if overwrite_colors is None:
            ...
            gaussian_colors = jt.clamp(sh2rgb + 0.5, min_v=0.0, max_v=1.0)
        else:
    ...
    def execute(self, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None,
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False,
        ...
        if return_xblock:
            x_block,planes,planes_sketch = self.predict_planes(ws, cache_backbone=cache_backbone,
                                            use_cached_backbone=use_cached_backbone,
                                            alpha_plane_resolution=alpha_plane_resolution,return_xblock=return_xblock, **synthesis_kwargs)
        elif planes_res==None:
            planes,planes_sketch = self.predict_planes(ws, cache_backbone=cache_backbone,
                                            use_cached_backbone=use_cached_backbone,
        ...
        if self._config.use_background_cnn:
            ...
            if neural_rendering_resolution > background_rgb.shape[-1]:
                background_rgb = nn.interpolate(background_rgb, size=(
        ...
        #-----------------------------------------------------
        gaussiansketch_attribute_output = self.predict_gaussian_attributes(
            planes_sketch,
            return_raw_attributes=return_raw_attributes,
            return_uv_map=return_uv_map)
        gaussiansketch_attributes = gaussiansketch_attribute_output.gaussian_attributes
        #------------------------------------------------------
        ...
        #--------------------------------------------------------
        gaussiansketch_colors = gaussiansketch_attributes[GaussianAttribute.COLOR]
        gaussiansketch_scales = gaussiansketch_attributes[GaussianAttribute.SCALE]
        gaussiansketch_colors = gaussiansketch_colors.view(B, G, -1, C)
        # Gradient logging
        ...
        backgrounds = []
        for i in range(B):
            cam_2_world_pose = Pose(cam2world_matrix[i].numpy(), pose_type=PoseType.CAM_2_WORLD,
                                    disable_rotation_check=True)
            intrinsics = Intrinsics(intrinsics_matrix[i].numpy())
            ...
            self._gaussian_model.screenspace_points = jt.zeros_like(gaussian_positions[i]) + 0
            ...
            self._gaussian_model._rotation = gaussian_rotations[
                i].contiguous()  # Rotation needs to be contiguous!
            self._gaussian_model._opacity = gaussian_opacities[i]
            #--------------------------------------------------------------------------------------
            self._gaussian_model._features_dc = gaussiansketch_colors[i][:, [0]]
            self._gaussian_model._features_rest = gaussiansketch_colors[i][:, 1:]  # [G, SH-1, 3]
            # self._gaussian_model._scaling = gaussiansketch_scales[i]
            #----------------------------------------------------------------------------
            if sh_ref_cam is not None:
                gaussian_sh_ref_cam = pose_to_rendercam(sh_ref_cam, intrinsics, neural_rendering_resolution, neural_rendering_resolution)
                ...
                override_color = None
            #-------------------------------------------------------------------------------
            gaussian_bg = jt.zeros_like(self._gaussian_bg_train)
            rendered_sketch = render(gaussian_camera, self._gaussian_model, PipelineParams2(), gaussian_bg,
                                    override_color=override_color)
            sketch = 1-rendered_sketch['render']
            #-------------------------------------------------------------------------------------------
            self._gaussian_model._features_dc = gaussian_colors[i][:, [0]]
            self._gaussian_model._features_rest = gaussian_colors[i][:, 1:]
            self._gaussian_model._scaling = gaussian_scales[i]
            if sh_ref_cam is not None:
                gaussian_sh_ref_cam = pose_to_rendercam(sh_ref_cam, intrinsics, neural_rendering_resolution, neural_rendering_resolution)
            ...
            if self._config.use_background_cnn or return_masks:
                ...
                black_colors = jt.ones_like(gaussian_colors[i][:, 0]) * 0
            ...
            camera.append(gaussian_camera)
            sketchs.append(sketch)
        rgb_images_direct = jt.stack(rgb_images)
        sketchs = jt.stack(sketchs)
        # masks = jt.stack(masks) if self._config.use_masks or return_masks else None
        ...
        output['sketchs'] = sketchs
        if return_xblock:
            return x_block,output
        return output

    def _setup_gaussian_model(self, gaussian_attributes: Dict[GaussianAttribute, jt.Var],
                              i: int) -> GaussianModel:
        gaussian_positions = gaussian_attributes[GaussianAttribute.POSITION]
        gaussian_scales = gaussian_attributes[GaussianAttribute.SCALE]
        gaussian_rotations = gaussian_attributes[GaussianAttribute.ROTATION]
    ...
    def _collect_gaussian_attributes(self, attribute_names: List[GaussianAttribute], predictions: jt.Var,
                                     return_raw_attributes: bool = False) \
            -> Tuple[Dict[GaussianAttribute, jt.Var], Dict[GaussianAttribute, jt.Var]]:
        gaussian_attributes = dict()
        raw_gaussian_attributes = dict()
        c = 0
    ...
    def _apply_gaussian_attribute_activation(self, attribute_name: GaussianAttribute, value: jt.Var) -> jt.Var:
        ...
        # SCALE
            ...
            if self._config.center_scale_activation:
                value = self._config.max_scale - nn.softplus(value + self._config.max_scale)
            else:
                value = self._config.max_scale - nn.softplus(value)
        ...
        # ROTATION
            ...
            if self._config.gaussian_attribute_config.use_rodriguez_rotation:
                ...
                if self._config.use_rotation_activation:
                    value = jt.tanh(self._config.rotation_attenuation * value) * (2 * np.pi)
                value = axis_angle_to_quaternion(value)
                value = jt.concat([value[..., [3]], value[..., :3]], dim=-1)  # xyzr -> rxyz
    ...
    def _apply_position_activation(self, value: jt.Var) -> jt.Var:
        ...
        if self._config.use_position_activation:
            value = self._config.position_range * jt.tanh(self._config.position_attenuation * value)
        else:
            value = self._config.position_attenuation * value
        return value

    def _apply_opacity_activation(self, value: jt.Var) -> jt.Var:
        return mip_sigmoid(value, overshoot=self._config.opacity_overshoot, clamp=self._config.clamp_opacity)

    def _apply_color_activation(self, value: jt.Var) -> jt.Var:
    ...
    def _apply_cnn_color_activation(self, value: jt.Var) -> jt.Var:
        return mip_tanh2(value, clamp=True)

    def _apply_scale_activation(self, value: jt.Var) -> jt.Var:
