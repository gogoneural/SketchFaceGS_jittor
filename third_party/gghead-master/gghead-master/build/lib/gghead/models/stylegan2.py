...
class GGHSynthesisBlock(nn.Module):
    ...
    def forward(self, x, img, ws, w_rgb=None, w_rgb2=None, x_block=None,x_block2=None,mask_gt=None,conditions=None,force_fp32=False, fused_modconv=None, update_emas=False, alpha_new_layers: float = 1, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            if mask_gt!=None:
                ...
                mask_gt_ = (mask_gt_ > 0.9)   # 或者用 >= 0.5
                x = mask_gt_ * x + ~mask_gt_ * x_block
            x_=x.clone()
        ...
        with torch.autograd.profiler.record_function('split_ws'):
            ...
            for res in self.block_resolutions:
                ...
                if isinstance(conditions, dict):
                    ...
                    block_ws_rgb2.append(conditions['w'].narrow(1, w_idx+block.num_conv-1, block.num_torgb))
        ...
        for idx,(res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)):
            ...
            if self.pretrained_plane_resolution is not None and res > self.pretrained_plane_resolution:
                ...
                if conditions!=None and idx!=0:
                    if isinstance(conditions, dict):
                        if idx==6:
                            x, img, x_ = block(x, img, cur_ws, conditions=conditions['conditions'][2*idx-2:2*idx],mask_gt=mask_gt[idx], 
                                       x_block=conditions['x_block'][idx],x_block2=conditions['x_block2'][idx],w_rgb = block_ws_rgb[idx],w_rgb2 = block_ws_rgb2[idx],**block_kwargs)
                        else:
                            x, img, x_ = block(x, img, cur_ws, conditions=conditions['conditions'][2*idx-2:2*idx],mask_gt=mask_gt[idx], 
                                       x_block=conditions['x_block'][idx],x_block2=conditions['x_block2'][idx],w_rgb = block_ws_rgb[idx],w_rgb2 = block_ws_rgb2[idx],**block_kwargs)
                    else:
                        x, img, x_ = block(x, img, cur_ws, conditions=conditions[2*idx-2:2*idx], **block_kwargs)
                else:
                    if isinstance(conditions, dict):
                        x, img, x_ = block(x, img, cur_ws, mask_gt=mask_gt[idx], x_block=conditions['x_block'][idx],x_block2=conditions['x_block2'][idx],w_rgb = block_ws_rgb[idx],w_rgb2 = block_ws_rgb2[idx],**block_kwargs)
                    else:    
                        x, img, x_ = block(x, img, cur_ws,  **block_kwargs)
            x_block.append(x_)
            x_block2.append(x)
        if return_xblock:
            return {'x_block':x_block,'x_block2':x_block2}, img
        return img
