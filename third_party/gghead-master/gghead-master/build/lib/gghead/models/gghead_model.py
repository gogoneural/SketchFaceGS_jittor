...
class GGHeadOutput(dict):
    ...
    def keys(self):
        return iter(['image', 'image_raw', 'image_depth', 'camera','gaussian_attribute_output','planes'])
...
class GGHeadModel(nn.Module):
    ...
    def predict_planes(self, ws: torch.Tensor, update_emas=False, cache_backbone=False, use_cached_backbone=False,
        ...
        if use_cached_backbone and self._last_planes is not None:
            ...
            x_block,planes = self.backbone.synthesis(ws, update_emas=update_emas,return_xblock=return_xblock, **synthesis_kwargs)
        else:
            planes = self.backbone.synthesis(ws, update_emas=update_emas,return_xblock=return_xblock, **synthesis_kwargs)
        if cache_backbone:
            self._last_planes = planes
        if return_xblock:
            return x_block,planes
        return planes
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False,
        ...
        if return_xblock:
            x_block,planes = self.predict_planes(ws, update_emas=update_emas, cache_backbone=cache_backbone,
            ...
            planes = self.predict_planes(ws, update_emas=update_emas, cache_backbone=cache_backbone,
                                            use_cached_backbone=use_cached_backbone,
                                            alpha_plane_resolution=alpha_plane_resolution, **synthesis_kwargs)+planes_res
        if self._config.use_background_cnn:
