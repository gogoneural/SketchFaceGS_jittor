...
class GGHeadOutput(dict):
    ...
    planes: torch.tensor = None
    sketchs: torch.tensor = None
    ...
    def __setitem__(self, key, value):
        if key == 'image':
            ...
            if not isinstance(value, list):
                raise TypeError("Camera must be a list of torch.Tensor.")
            self.camera = value
        elif key == 'gaussian_attribute_output':
...
class GGHeadModel(nn.Module):
    ...
    def __init__(self, config: GGHeadConfig, logger_bundle: Optional[LoggerBundle] = None,
        ...
        # self.register_buffer("_idxim", idxim.contiguous())
        # self.register_buffer("_barim", barim.contiguous())
    ...
    def gghead_updata(self):
        ...
        unique_uv = torch.unique(self._uv_idx, dim=0)
        ...
        # 继续使用处理后的 mesh 进行计算
        plane_resolution = 256
        # self._gaussian_bg_train = torch.zeros_like(self._gaussian_bg_train)
        idxim, tidxim, barim = gen_tritex(unique_uvs, new_faces, new_faces, plane_resolution)
        self.register_buffer("_idxim", torch.tensor(idxim).contiguous())
        self.register_buffer("_barim", torch.tensor(barim).contiguous())
        self.register_buffer("_vertices", torch.tensor(unique_vertices).contiguous())
        # self.register_buffer("_uv_coords", torch.tensor(unique_uvs).contiguous())
        self.register_buffer("_faces", torch.tensor(new_faces).contiguous())
    ...
    def predict_planes(self, ws: torch.Tensor, update_emas=False, cache_backbone=False, use_cached_backbone=False,
        ...
        if use_cached_backbone and self._last_planes is not None:
            ...
            x_block,planes,planes_sketch = self.backbone.synthesis(ws, update_emas=update_emas,return_xblock=return_xblock, **synthesis_kwargs)
        else:
            planes,planes_sketch = self.backbone.synthesis(ws, update_emas=update_emas,return_xblock=return_xblock, **synthesis_kwargs)
        if cache_backbone:
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False,
        ...
        if return_xblock:
            x_block,planes,planes_sketch = self.predict_planes(ws, update_emas=update_emas, cache_backbone=cache_backbone,
            ...
            planes,planes_sketch = self.predict_planes(ws, update_emas=update_emas, cache_backbone=cache_backbone,
                                            use_cached_backbone=use_cached_backbone,
        ...
        for i in range(B):
            ...
            if sh_ref_cam is not None:
                ...
                override_color = None
            #-------------------------------------------------------------------------------
            gaussian_bg = torch.zeros_like(self._gaussian_bg_train)
            with torch.cuda.device(device):
