...
def marching_cubes_with_contraction(
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                ...
                x = jt.linspace(x_min, x_max, cropN)
                y = jt.linspace(y_min, y_max, cropN)
                z = jt.linspace(z_min, z_max, cropN)

                xx, yy, zz = jt.meshgrid(x, y, z, indexing="ij")
                points = jt.array(jt.vstack([xx, yy, zz]).T, dtype="float")

                # @torch.no_grad()
                def evaluate(points):
                    z = []
                    for _, pnts in enumerate(jt.split(points, 256**3, dim=0)):
                        z.append(sdf(pnts))
                    z = jt.cat(z, axis=0)
                    return z
    ...
    if inv_contraction is not None:
        combined.vertices = inv_contraction(jt.array(combined.vertices).float()).numpy()
