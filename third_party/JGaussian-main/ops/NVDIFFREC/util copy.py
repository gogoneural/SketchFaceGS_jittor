...
def latlong_to_cubemap_ori(latlong_map, res):
...
def cubemap_to_latlong_ori(cubemap, res):
...
def latlong_to_cubemap(latlong_map, res):
    ...
    for s in range(6):
        ...
        #                         torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cuda'), 
        #                         )
        gy, gx = torch.meshgrid(torch.linspace(-1.0, 1.0, res[0], device='cuda'), 
                                torch.linspace(-1.0, 1.0, res[1], device='cuda'), 
                                )
        v = safe_normalize(cube_to_dir(s, gx, gy))

        tu = torch.atan2(v[..., 0:1], v[..., 1:2]) / (2 * np.pi) + 0.25
...
def cubemap_to_latlong(cubemap, res):
    gy, gx = torch.meshgrid(torch.linspace( 0.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device='cuda'), 
                            torch.linspace(1.5 - 1.0 / res[1], -0.5 + 1.0 / res[1], res[1], device='cuda'),
                            # indexing='ij')
                            )
