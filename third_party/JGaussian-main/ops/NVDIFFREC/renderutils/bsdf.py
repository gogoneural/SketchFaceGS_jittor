...
def _safe_normalize(x):
    return jt.normalize(x, dim = -1)
    # return jt.nn.functional.normalize(x, dim = -1)
...
def _bend_normal(view_vec, smooth_nrm, geom_nrm, two_sided_shading):
    ...
    if two_sided_shading:
        smooth_nrm = jt.where(_dot(geom_nrm, view_vec) > 0, smooth_nrm, -smooth_nrm)
        geom_nrm   = jt.where(_dot(geom_nrm, view_vec) > 0, geom_nrm, -geom_nrm)

    t = jt.clamp(_dot(view_vec, smooth_nrm) / NORMAL_THRESHOLD, min=0, max=1)
    return jt.lerp(geom_nrm, smooth_nrm, t)
    # return jt.lerp(geom_nrm, smooth_nrm, t)

def _perturb_normal(perturbed_nrm, smooth_nrm, smooth_tng, opengl):
    smooth_bitang = _safe_normalize(jt.cross(smooth_tng, smooth_nrm))
    if opengl:
        shading_nrm = smooth_tng * perturbed_nrm[..., 0:1] - smooth_bitang * perturbed_nrm[..., 1:2] + smooth_nrm * jt.clamp(perturbed_nrm[..., 2:3], min=0.0)
    else:
        shading_nrm = smooth_tng * perturbed_nrm[..., 0:1] + smooth_bitang * perturbed_nrm[..., 1:2] + smooth_nrm * jt.clamp(perturbed_nrm[..., 2:3], min=0.0)
...
def bsdf_lambert(nrm, wi):
    return jt.clamp(_dot(nrm, wi), min=0.0) / math.pi
...
def bsdf_frostbite(nrm, wi, wo, linearRoughness):
    ...
    return jt.where((wiDotN > 0.0) & (woDotN > 0.0), res, jt.zeros_like(res))
...
def bsdf_phong(nrm, wo, wi, N):
    dp_r = jt.clamp(_dot(_reflect(wo, nrm), wi), min=0.0, max=1.0)
    dp_l = jt.clamp(_dot(nrm, wi), min=0.0, max=1.0)
...
def bsdf_fresnel_shlick(f0, f90, cosTheta):
    _cosTheta = jt.clamp(cosTheta, min=specular_epsilon, max=1.0 - specular_epsilon)
...
def bsdf_lambda_ggx(alphaSqr, cosTheta):
    ...
    res = 0.5 * (jt.sqrt(1 + alphaSqr * tanThetaSqr) - 1.0)
    return res
...
def bsdf_pbr_specular(col, nrm, wo, wi, alpha, min_roughness=0.08):
    _alpha = jt.clamp(alpha, min=min_roughness*min_roughness, max=1.0)
    ...
    w = F * D * G * 0.25 / jt.clamp(woDotN, min=specular_epsilon)

    frontfacing = (woDotN > specular_epsilon) & (wiDotN > specular_epsilon)
    return jt.where(frontfacing, w, jt.zeros_like(w))
