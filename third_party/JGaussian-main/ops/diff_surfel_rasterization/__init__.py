...
class _RasterizeGaussians(jt.Function):
    ...
    def execute(self,
        ...
        if raster_settings.debug:
            try:
                num_rendered, color, depth ,radii, geomBuffer, binningBuffer, imgBuffer = rasterize_points.RasterizeGaussiansCUDA(*args)
            except Exception as ex:
    ...
    def grad(self,grad_out_color, grad_radii, grad_depth):
        if(grad_out_color is None):
            grad_out_color = jt.zeros((3,grad_depth.shape[1],grad_depth.shape[2]))
        if(grad_depth is None):
            grad_depth = jt.zeros((7,grad_out_color.shape[1],grad_out_color.shape[2]))
        # print(grad_depth.nonzero())
        ...
        if raster_settings.debug:
            ...
            # ic(grad_means2D)
            # ic(grad_colors_precomp)
            # ic(grad_means3D)
            # ic(grad_cov3Ds_precomp)
            # ic(grad_sh)
            # ic(grad_scales)
            # ic(grad_rotations)
...
class GaussianRasterizer(nn.Module):
    ...
    def execute(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
        ...
        return self.rasterizeFunc.apply(
            means3D,
            means2D,
            shs,
            colors_precomp,
