...
__device__ __forceinline__ float atomicMaxFloat (float * addr, float value) {
    float old;
    old = (value+0 >= 0) ? __int_as_float(atomicMax((int *)addr, __float_as_int(value))) :
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA_apply_weights(
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      // Always accumulate C[ch] (even when 0) and count ALL contributing pixels,
      // so that weights/cnt = average image_weight across all pixels this Gaussian
      // contributes to. This matches the PyTorch apply_weights semantics where cnt
      // counts every pixel the Gaussian touches, not just image_weights>0 pixels.
      for (int ch = 0; ch < CHANNELS; ch++) {
